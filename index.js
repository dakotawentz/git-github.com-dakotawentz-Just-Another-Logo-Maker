...
function writeToFile(fileName, answers) {
    ...
    if (answers.shape === "Circle") {
        ...
        svgString += `<circle cx="150" cy="115" r="80" fill="${this.shapeBackgroundColor}" />`
    } else if (answers.shape === "Triangle") {
        shapeChoice = new Triangle();
        svgString += `<polygon points="150,18 244,182 56,182" fill="${this.shapeBackgroundColor}" />`;
    } else {
        shapeChoice = new Square();
        svgString += `<rect x="73" y="40" width="160" height="160" fill="${this.shapeBackgroundColor}" />`;
    }    
